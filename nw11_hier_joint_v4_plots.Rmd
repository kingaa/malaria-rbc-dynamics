...
group.fits |>
  filter(pABA!="control",name%in%c("R","E")) |>
  ggplot(aes(x=time,y=value,color=pABA))+
  geom_line(linewidth=2)+
  scale_y_log10()+
  xlab("Days")+ylab("Density per microlitre")+
...
po_df |>
  ...
  mutate(K=(logR+logE)*(1-exp(-logM/(logR+logE))),
         RBC=logR+logE) |>
  select(time,mouse,box,paba=pABA,RBC,E=logE,R=logR,K,N=logN,W=logW) -> swirls2

ggplot()+
  geom_point(data=data_full |>filter(day<=20,paba!="control",paba!="0.05"),aes(x=day,y=Retic,col=mouse))+
  geom_line(data=swirls2 |> filter(mouse!="GROUP",paba!="control",paba!="0.05"),aes(x=time,y=R,col=mouse))+
  xlab("Day")+ylab("Reticulocyte supply")+
  facet_grid(paba~.)+
  theme_bw()+
  scale_y_log10()+
  theme(
