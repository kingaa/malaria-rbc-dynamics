...
create_objfun <- function (
  ...
  idx <- match(est,names(params4)) #vector of positions
  ...
  ofun <- function (par) {
    ...
    fit4$value+fit_control$value
  }
  environment(ofun) <- list2env(
    list(object4=object4,params4=params4,coefs4=coefs4,
...
stew(
  file="nw11_hier_joint_box04control.rda",
  info=TRUE,
  {
    create_objfun(
      object4=po04,
    ...
    try({
      optim(
        fn=ofun,
        par=coef(po04,c("alphaw","alphan","alphar",
                        "sigmaw","sigman","sigmar",
...
est.coefs<-bind_rows(est.coefs04,est.coefs05)

write.csv(est.coefs,"est_coefs_joint_box04control.csv",row.names=FALSE)
...
est.po<-bind_rows(est.po04,est.po05)
write.csv(est.po,"est_po_joint_box04control.csv",row.names=FALSE)
