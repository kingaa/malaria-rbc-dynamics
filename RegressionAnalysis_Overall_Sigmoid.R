...
sigmoid <- function(lagRBC,F0,theta,logk){
  #F0*(theta^k)/(theta^k+lagRBC^k) #Hill
  #(F0/2)*(tanh((-lagRBC+theta)/10^exp(logk))+1) #tanh
  #(F0/(1+exp(-(10^(-exp(logk))*(-lagRBC+theta))))) #logistic
  F0/(1+exp((k*(lagRBC-theta))/1e6))
}
...
stats_df |> write_csv("results_regression_stats_sigmoid.csv")
## preds_df |> write_csv("results_regression_preds.csv")
quants_df |> write_csv("results_regression_quants_sigmoid.csv")
