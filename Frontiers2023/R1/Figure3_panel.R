...
lag2 <- group_traj |>
  ...
  labs(x=expression(paste("Erythrocyte density at time ", italic("t"), "-2 (density per µL)")),
       y=expression(paste("Reticulocyte supply at time ", italic("t"), " (density per µL)"))
  )+
  labs(colour="Parasite nutrient (pABA)")+
  theme_bw()+
...
lag3 <- group_traj |>
  ...
  labs(x=expression(paste("Erythrocyte density at time ", italic("t"), "-3 (density per µL)")),
       y=expression(paste("Reticulocyte supply at time ", italic("t"), " (density per µL)"))
  )+
...
p
ggsave("Figure3_panel.jpeg",width=40,height=14,units="cm")
