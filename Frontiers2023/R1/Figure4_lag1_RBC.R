...
bake(file="results_df_RBC_lag1.rds",{
  foreach (
    r=1:rep_num,
    .combine=rbind,
...
second_model_plot <- results_df_RBC |>
  ...
  filter(model_b=="m3, 9") |>
  ggplot()+
  geom_line(aes(x=lagRBC,y=pred,group=interaction(r,phase),col=phase),alpha=0.05)+
...
ggsave("Figure4_lag1_RBC.jpeg",width=25,height=30,units="cm")
