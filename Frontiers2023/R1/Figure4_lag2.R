...
bake(file="results_df_RE_lag2.rds",{
foreach (
  r=1:rep_num,
  .combine=rbind,
...
results_bar$label <- factor(results_bar$model_b,
                            levels=c("m6, 9","m5, 9","m3, 9","m2, 9","m1, 10","m4, 9","m6, 10","m5, 10","m2, 10","m3, 10","m1, 9","m4, 10"),
                            labels=c("Model F,\nbreakpoint 9",
                                     "Model E,\nbreakpoint 9",
                                     "Model C,\nbreakpoint 9",
                                     "Model B,\nbreakpoint 9",
...
(bar_plot <- freq_df |>
  ...
  annotate("text",x=13,y=0,label="X",size=5)+
  
  annotate("text",x=2,y=0.55,label=expression(paste("Model F: ",
                                                    R[t]%~%E[t-2]:phase+phase)),size=5,hjust=0,parse=T)+ 
  annotate("text",x=2,y=0.35,label=expression(paste("Model C: ",
                                                    R[t]%~%E[t-2]^{2}:phase+E[t-2]:phase+phase)),size=5,hjust=0,parse=T)+ 
  annotate("text",x=2,y=0.45,label=expression(paste("Model E: ",
                                                    R[t]%~%E[t-2]:phase+pABA:phase+phase)),size=5,hjust=0,parse=T)+
    annotate("text",x=2,y=0.25,label=expression(paste("Model B: ",
                                                      R[t]%~%E[t-2]^{2}:phase+E[t-2]:phase+pABA:phase+phase)),size=5,hjust=0,parse=T)+ 
  theme_bw()+
  theme(
...
gt2 <- arrangeGrob(best_model_plot, second_model_plot, third_model_plot, fourth_model_plot,
                   ...
                   bottom="Erythrocyte density (t-2)")
...
ggsave("Figure4_lag2.jpeg",width=25,height=30,units="cm")
