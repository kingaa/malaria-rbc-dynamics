...
bake(file="results_df_RE_lag3.rds",{
foreach (
  r=1:rep_num,
  .combine=rbind,
...
results_bar$label <- factor(results_bar$model_b,
                            levels=c("m6, 11","m3, 11","m5, 11","m1, 11","m2, 11","m4, 11","m6, 9","m2, None","m1, None","m6, 8"),
                            labels=c("Model F,\nbreakpoint 11",
                                     "Model C,\nbreakpoint 11",
                                     "Model E,\nbreakpoint 11",
                                     "Model A,\nbreakpoint 11",
...
(bar_plot <- freq_df |>
  ...
  annotate("text",x=2,y=0.55,label=expression(paste("Model F: ",
                                                    R[t]%~%E[t-3]:phase+phase)),size=5,hjust=0,parse=T)+ 
  annotate("text",x=2,y=0.35,label=expression(paste("Model C: ",
                                                    R[t]%~%E[t-3]^{2}:phase+E[t-3]:phase+phase)),size=5,hjust=0,parse=T)+ 
  annotate("text",x=2,y=0.45,label=expression(paste("Model E: ",
                                                    R[t]%~%E[t-3]:phase+pABA:phase+phase)),size=5,hjust=0,parse=T)+
    annotate("text",x=2,y=0.25,label=expression(paste("Model A: ",
                                                      R[t]%~%E[t-3]^{2}:pABA:phase+E[t-3]:pABA:phase+pABA:phase)),size=5,hjust=0,parse=T)+ 
  theme_bw()+
  theme(
...
best_model_plot <- results_df_RE |>
  ...
  geom_text(aes(x=7500000,y=3900000,label="Model F, breakpoint 11"))+
  theme_bw()+
  theme(
...
second_model_plot <- results_df_RE |>
    ...
    scale_y_continuous(labels = aakmisc::scinot,limits=c(0,4000000))+
    scale_colour_manual(values=cbPalette[c(6,8)])+
    geom_text(aes(x=7500000,y=3900000,label="Model C, breakpoint 11"))+
    theme_bw()+
    theme(
      axis.title=element_blank(),
...
third_model_plot <- results_df_RE |>
    ...
    geom_text(aes(x=7500000,y=3900000,label="Model E, breakpoint 11"))+
...
fourth_model_plot <- results_df_RE |>
    ...
    scale_colour_manual(values=cbPalette[c(2:5)])+
    guides(colour = guide_legend(override.aes = list(alpha = 1)))+
    geom_text(aes(x=7500000,y=3900000,label="Model A, breakpoint 11"))+
...
gt2 <- arrangeGrob(best_model_plot, second_model_plot, third_model_plot, fourth_model_plot,
                   ...
                   bottom="Erythrocyte density (t-3)")
...
ggsave("Figure4_lag3.jpeg",width=25,height=30,units="cm")
