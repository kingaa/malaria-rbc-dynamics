...
best_model_plot <- results_df_RBC |>
  ...
  annotate("text",x=5000000,y=1000000,label="Phase 1",size=5)+
  geom_segment(aes(x=5000000,xend=5500000,y=1100000,yend=1350000))+
  geom_segment(aes(x=7000000,xend=6350000,y=3000000,yend=1550000))+
...
third_model_plot <- results_df_RBC |>
  ...
  filter(model_b=="m2, 9") |>
  ggplot()+
  geom_line(aes(x=lagRBC,y=pred,group=interaction(r,phase,box),col=pABA),alpha=0.05)+
...
fourth_model_plot <- results_df_RBC |>
  ...
  filter(model_b=="m5, 9") |>
  ggplot()+
  geom_line(aes(x=lagRBC,y=pred,group=interaction(r,phase,box),col=pABA),alpha=0.07)+
...
ggsave("Figure4_lag3_RBC.jpeg",width=25,height=30,units="cm")
