---
title: "FDA results - 3 boxes, missing data omitted"
...
## Load convergence and likelihood

```{r ConLik, echo=FALSE}
rda.obj <- load("FDA_3Boxes.rda")
print(paste("The convergence integer code is ",fit$convergence,".",sep=""))
print(paste("The log likelihood at convergence (given the integer code is zero) is ",round(-fit$value,2),".",sep=""))
```

## Load fits and parameter estimates

```{r fits, echo=FALSE}
coef_df <- read_csv("parameter_results_3Boxes.csv",show_col_types = FALSE)
po_df <- read_csv("trajectory_results_3Boxes.csv",show_col_types = FALSE) |>
  mutate(
  pABA=case_match(
    box,
    "01"~"5%",
...
po_df |> pivot_wider(names_from=name,values_from=value) |>
  ...
  select(Mouse,time,box,pABA,Reticulocytes,Merozoites,`Indiscriminate killing`,
