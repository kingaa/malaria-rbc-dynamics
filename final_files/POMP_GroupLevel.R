...
#Read in PNAS data
read_csv("~/Documents/GitHub/bdd/code/data.csv",
         col_types="iiinnnn"
) %>%
  mutate(
...
#Obtain MLE for beta and dose
pf4name <- "~/Documents/GitHub/bdd/code/results/m5pf4.rds"
pf4 <- readRDS(pf4name)
...
sm1name <- "~/Documents/GitHub/bdd/code/results/m5sm1.rds"
sm1 <- readRDS(sm1name)
...
#Plot group-level trajectories for all variables
group_traj |>
...
swirls |>
  ...
  scale_x_log10()+scale_y_log10()+
  scale_colour_manual(values=cbPalette)+
  xlab("Reticulcoytes")+ylab("Indiscriminate killing")+
  theme_bw()+
  theme(
    panel.grid = element_blank()
  )
...
swirls |>
  ...
  scale_x_log10()+scale_y_log10()+
  scale_colour_manual(values=cbPalette)+
  xlab("Reticulocytes")+ylab("Targeted killing")+
  theme_bw()+
  theme(
    panel.grid = element_blank()
  )
