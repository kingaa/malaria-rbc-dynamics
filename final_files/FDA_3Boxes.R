...
create_objfun <- function (
  ...
  idx <- match(est,names(params2)) #vector of positions
  ...
  ofun <- function (par) {
    ...
    fit2$value+fit4$value+fit_control$value
  }
  environment(ofun) <- list2env(
    list(object2=object2,params2=params2,coefs2=coefs2,
...
est.coefs<-bind_rows(est.coefs02,est.coefs04,est.coefs05)

write.csv(est.coefs,"parameter_results_3Boxes.csv",row.names=FALSE)
...
est.po<-bind_rows(est.po02,est.po04,est.po05)
write.csv(est.po,"trajectory_results_3Boxes.csv",row.names=FALSE)
