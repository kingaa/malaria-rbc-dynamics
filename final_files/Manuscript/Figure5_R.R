...
(second_model_plot <- results_df_R |>
    ...
    ggtitle("Reticulocyte supply ~ time")+
    ylim(range(results_df_R$pred))+
    theme_bw()+
    theme(
      axis.title=element_text(size=15),
      axis.text=element_text(size=11),
...
(best_model_plot <- results_df_R |>
    ...
    xlab("Day post-infection")+ylab("Reticulocyte supply (density per μL)")+
    scale_colour_manual(values=cbPalette[2:5])+
    ggtitle("Reticulocyte supply ~ time x pABA")+
...
(best_model_facet <- results_df_R |>
    ...
    ggplot()+
    geom_line(aes(x=time,y=pred,group=interaction(r,pABA),col=pABA),alpha=0.03)+
    xlab("Day post-infection")+ylab("Reticulocyte supply")+
    scale_colour_manual(values=cbPalette[2:5])+
    ylim(range(results_df_R$pred))+
    facet_grid(.~pABA)+
    theme_bw()+
...
p
ggsave("Figure5_R.png",width=22,height=16,units="cm")
