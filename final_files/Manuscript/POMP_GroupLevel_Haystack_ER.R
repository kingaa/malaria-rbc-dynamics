...
(haystack_facet <- ggplot()+
  geom_path(data=pred |> filter(lm==1),aes(x=lagE,y=predR,group=key,col=pABA),alpha=0.025)+
  geom_path(data=pred |> filter(lm==2),aes(x=lagE,y=predR,group=key,col=pABA),alpha=0.025)+
  facet_wrap(pABA~.)+
...
ggsave("haystack_facet.png",
       width=20,
       height=20,
       units=c("cm"))

haystack_stacked_legend <- get_legend(ggplot()+
  geom_line(data=pred |> filter(lm==1),aes(x=lagE,y=predR,group=key,col=pABA))+
  geom_line(data=pred |> filter(lm==2),aes(x=lagE,y=predR,group=key,col=pABA))+
  scale_colour_manual(values=cbPalette[2:5])+
  scale_y_continuous(labels = scientific)+
  scale_x_continuous(labels = scientific)+
  facet_grid(.~phase)+
...
(haystack_stacked <- ggplot()+
  geom_line(data=pred |> filter(lm==1),aes(x=lagE,y=predR,group=key,col=pABA),alpha=0.025)+
  geom_line(data=pred |> filter(lm==2),aes(x=lagE,y=predR,group=key,col=pABA),alpha=0.025)+
  scale_colour_manual(values=cbPalette[2:5])+
  scale_y_continuous(labels = scientific)+
  scale_x_continuous(labels = scientific)+
  facet_grid(.~phase)+
...
haystacked_stacked_arranged <- ggarrange(haystack_stacked, haystack_stacked_legend, ncol = 2, nrow = 1,
          widths=c(1,0.2))

ggsave("haystack_stacked.png",
       plot=haystacked_stacked_arranged,
       width=20,
       height=10,
       units=c("cm"))
