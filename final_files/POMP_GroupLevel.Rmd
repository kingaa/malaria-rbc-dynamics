...
```{r WversusKwithError, echo=FALSE}

group_traj |>
  filter(variable%in%c("K","W")) |>
  filter(time%in%5:7) |>
  pivot_longer(lo:hi) |>
  pivot_wider(names_from=c(variable,name),values_from=value,names_sep="_") -> WKerr

ggplot()+
  geom_errorbar(data=WKerr,aes(x=K_med,ymin = W_lo,ymax = W_hi,col = pABA))+
  geom_errorbarh(data=WKerr,aes(y=W_med,xmin = K_lo,xmax = K_hi,col=pABA))+
  geom_path(data=swirls|>filter(time%in%5:7),aes(x=K,y=W,color=pABA),linewidth=2)+
  geom_text(data=swirls |> filter(time%in%c(5,6,7)),aes(x=K,y=W,label=time),col="black",size=5)+
  scale_x_log10()+scale_y_log10()+
  scale_colour_manual(values=cbPalette)+
  xlab("Parasite density")+ylab("Targeted killing")+
  theme_bw()+
...
```{r NversusRwithError, echo=FALSE}
...
ggplot()+
  geom_errorbar(data=NRerr,aes(x=R_med,ymin = N_lo,ymax = N_hi,col = pABA))+
  geom_errorbarh(data=NRerr,aes(y=N_med,xmin = R_lo,xmax = R_hi,col=pABA))+
  geom_path(data=swirls|>filter(time<=6),aes(x=R,y=N,color=pABA),linewidth=2)+
  geom_text(data=swirls |> filter(time<=6),aes(x=R,y=N,label=time),col="black",size=5)+
  scale_x_log10()+scale_y_log10()+
  scale_colour_manual(values=cbPalette)+
  xlab("Reticulocyte supply")+ylab("Indiscriminate killing")+
  theme_bw()+
...
```{r lambdaUtolambdaW, echo=FALSE}
group_traj |>
  filter(time<=20,variable%in%c("lambda_u_w_ratio")) |>
  mutate(variable=case_match(variable,
                             "lambda_u_w_ratio"~"lambda_u:lambda_w")) |>
  ggplot()+
  geom_line(aes(x=time,y=med,color=pABA),linewidth=2)+
  ...
  xlab("Day")+ylab(expression(lambda[U]:lambda[W]))+
  theme_bw()+
  theme(strip.background=element_blank(),
        panel.grid = element_blank()
  )
...
```{r lambdaUtolambdaWN, echo=FALSE}
group_traj |>
  filter(time<=20,variable%in%c("lambda_u_wn_ratio")) |>
  mutate(variable=case_match(variable,
                             "lambda_u_wn_ratio"~"lambda_u:(lambda_w+lambda_n)")) |>
  ggplot()+
  geom_line(aes(x=time,y=med,color=pABA),linewidth=2)+
  ...
  scale_colour_manual(values=cbPalette[2:7])+
  scale_fill_manual(values=cbPalette[2:7])+
  xlab("Day")+ylab(expression(lambda[U]:lambda[W]+lambda[N]))+
  theme_bw()+
  theme(strip.background=element_blank(),
        panel.grid = element_blank()
  )
...
```{r dataWeighted, echo=FALSE}
data_weighted <- read_csv("~/Documents/GitHub/bdd/sindy/data_weighted.csv")
...
```{r individualAndGroup, echo=FALSE}
group_traj |>
  select(-lo,-hi) |>
  pivot_wider(names_from="variable",values_from="med") -> group_traj_wide

ggplot()+
  geom_path(data=data_weighted,aes(x=K,y=W,group=interaction(rep,rep2),col=pABA),alpha=0.01)+
  geom_path(data=group_traj_wide,aes(x=K,y=W,col=pABA))+
  scale_x_log10()+scale_y_log10()+
  scale_colour_manual(values=cbPalette)+
  xlab("Parasite density")+ylab("Targeted killing")+
  theme_bw()+
...
ggplot()+
  geom_path(data=data_weighted,aes(x=R,y=W,group=interaction(rep,rep2),col=pABA),alpha=0.01)+
  geom_path(data=group_traj_wide,aes(x=R,y=W,col=pABA))+
  scale_x_log10()+scale_y_log10()+
  scale_colour_manual(values=cbPalette)+
  xlab("Reticulocyte supply")+ylab("Targeted killing")+
  theme_bw()+
...
ggplot()+
  geom_path(data=data_weighted,aes(x=R,y=N,group=interaction(rep,rep2),col=pABA),alpha=0.01)+
  geom_path(data=group_traj_wide,aes(x=R,y=N,col=pABA))+
  scale_x_log10()+scale_y_log10()+
  scale_colour_manual(values=cbPalette)+
  xlab("Reticulocyte supply")+ylab("Indiscriminate killing")+
  theme_bw()+
