...
create_objfun <- function (
  ...
  ofun <- function (par) {
    ...
    coefs2 <<- fit2$par
    coefs3 <<- fit3$par
    coefs_control <<- fit_control$par
    
    fit1$value+fit2$value+fit3$value+fit_control$value
  }
...
write.csv(est.coefs,"parameters_results_4Boxes.csv",row.names=FALSE)
...
write.csv(est.po,"trajectory_results_4Boxes.csv",row.names=FALSE)
